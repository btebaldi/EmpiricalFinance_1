...
cat(sprintf("Script que realiza o teste de Diebold Mariano para t-Student vs Normal para Expandind window 5"))
...
for (i in 1:length(fit.roll_5)) {
  # Get the predicitons and realized values.
  preds <- as.data.frame(fit.roll_5[[i]])
  
  tb_aux = tibble(Id = 1:nrow(preds), Date=NA, Model=NA, Dist=NA,  e= NA, q=NA)
  tb_aux$Date = rownames(preds)    
  
  tb_aux$Model = unlist(strsplit(names(fit.roll_5[i]), "\\."))[1]
  tb_aux$Dist = unlist(strsplit(names(fit.roll_5[i]), "\\."))[2]
...
# Comparacao de Normal vs t-student in fit.roll 5
tbl_temp = tbl %>% 
